...
const standards = [
  ["01", "Single donor", "Selected for consistency from root to tip."],
  ["02", "Raw & unaltered", "No chemical texture change. The natural character remains."],
    ...
    <div className="min-h-screen overflow-x-hidden bg-[#0b0907] text-[#f5f0e8] selection:bg-[#c8a45d] selection:text-[#0b0907]">
      ...
      <main>
          ...
          <motion.div style={{ opacity }} className="relative z-10 flex min-h-[100svh] items-end px-5 pb-10 md:px-10 md:pb-14 lg:px-16">
            <div className="grid w-full max-w-[1760px] gap-10 md:grid-cols-12 md:items-end">
              <div className="md:col-span-9">
                ...
                <h1 className="font-serif text-[clamp(4.2rem,11vw,13rem)] leading-[.68] tracking-[-.075em]">
                  <span className="block">The standard</span>
                  <span className="block pl-[8vw] italic text-[#c8a45d]">of</span>
                  <span className="block">beautiful hair.</span>
                </h1>
                <p className="mt-9 max-w-xl text-sm leading-7 text-white/65 md:text-base">Rare Vietnamese hair, deliberately selected and finished for women who expect presence without compromise.</p>
              </div>
          ...
          <div className="mx-auto max-w-[1760px]">
            <div className="grid gap-10 md:grid-cols-12 md:items-end">
              ...
              <div className="md:col-span-10">
                <h2 className="font-serif text-[clamp(3.5rem,7.7vw,9rem)] leading-[.75] tracking-[-.065em]">Luxury is not louder.<br /><i>It is more considered.</i></h2>
                <p className="mt-10 max-w-2xl text-sm leading-7 text-[#0b0907]/60 md:text-base">ByChi brings the discipline of a luxury atelier to Vietnamese hair. Origin, density, construction, movement and service are treated as part of one complete experience.</p>
              </div>
            </div>
            <div className="mt-20 grid border-y border-[#0b0907]/20 md:grid-cols-4">
              {[["01", "Origin", "Vietnam"], ["02", "Material", "Single donor"], ["03", "Finish", "Atelier"], ["04", "Care", "Private"]].map(([n, a, b]) => <div key={n} className="border-b border-[#0b0907]/15 p-7 last:border-0 md:border-b-0 md:border-r md:p-9 md:last:border-r-0"><span className="text-[8px] tracking-[.3em] text-[#c8a45d]">{n}</span><p className="mt-14 text-[9px] uppercase tracking-[.3em] text-[#0b0907]/45">{a}</p><h3 className="mt-2 font-serif text-3xl">{b}</h3></div>)}
            </div>
          </div>
        </section>
        ...
        <section className="bg-[#21150f] px-5 py-24 md:px-10 md:py-36">
          <div className="mx-auto max-w-[1760px]">
            <div className="grid gap-12 md:grid-cols-12 md:items-end"><p className="text-[8px] uppercase tracking-[.5em] text-[#c8a45d] md:col-span-2">03 / The Standard</p><div className="md:col-span-10"><h2 className="font-serif text-[clamp(3.5rem,7.5vw,9rem)] leading-[.74] tracking-[-.065em]">From Vietnam<br /><i>to Lagos.</i></h2><p className="mt-9 max-w-2xl text-sm leading-7 text-white/55 md:text-base">The journey matters. ByChi is built around traceable sourcing, natural texture and a considered atelier finish — not mass-market shortcuts.</p></div></div>
            <div className="mt-20 grid gap-px bg-[#c8a45d]/20 md:grid-cols-4">{standards.map(([n, title, copy]) => <article key={n} className="bg-[#21150f] p-7 md:p-10"><span className="text-[8px] tracking-[.3em] text-[#c8a45d]">{n}</span><h3 className="mt-16 font-serif text-3xl text-[#f5f0e8]">{title}</h3><p className="mt-5 text-sm leading-7 text-white/45">{copy}</p></article>)}</div>
          </div>
        </section>

        <section className="bg-[#f5f0e8] px-4 py-24 text-[#0b0907] md:px-8 md:py-36">
          <div className="mx-auto max-w-[1760px]">
            <div className="mb-12 flex items-end justify-between px-2"><div><p className="mb-4 text-[8px] uppercase tracking-[.5em] text-[#8b6244]">04 / The Pieces</p><h2 className="font-serif text-[clamp(3.5rem,7vw,8rem)] leading-[.74] tracking-[-.06em]">Made to be<br /><i>remembered.</i></h2></div><Link href="/shop" className="border-b border-[#c8a45d] pb-2 text-[8px] uppercase tracking-[.35em] text-[#8b6244]">Shop all</Link></div>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {[...collections, { name: "Colour Editions", label: "PRIVATE EDIT", image: "/products/IMG-20260727-WA0045.jpg", href: "/collections" }].map((item) => <Link key={item.name} href={item.href} className="group"><div className="relative aspect-[4/5] overflow-hidden bg-[#e6ddd0]"><Image src={item.image} alt={item.name} fill sizes="25vw" className="object-cover transition duration-1000 group-hover:scale-[1.04]" /></div><div className="flex items-start justify-between gap-4 pt-4"><div><p className="text-[8px] uppercase tracking-[.35em] text-[#8b6244]">{item.label}</p><h3 className="mt-2 font-serif text-2xl">{item.name}</h3></div><ArrowRight size={17} className="mt-2 text-[#c8a45d] transition group-hover:translate-x-1" /></div></Link>)}
            </div>
          </div>
        </section>

        <section className="bg-[#4a2a1d] px-5 py-24 md:px-10 md:py-36">
          <div className="mx-auto grid max-w-[1760px] gap-3 md:grid-cols-12 md:grid-rows-[260px_520px_260px]">
            <div className="relative overflow-hidden md:col-span-4 md:row-span-2"><Image src={editorial[0]} alt="ByChi hair detail" fill sizes="35vw" className="object-cover" /></div>
            <div className="relative overflow-hidden md:col-span-5"><Image src={editorial[1]} alt="ByChi editorial" fill sizes="40vw" className="object-cover" /></div>
            <div className="flex items-end p-6 md:col-span-3"><div><p className="mb-5 text-[8px] uppercase tracking-[.5em] text-[#c8a45d]">05 / The Atelier</p><h2 className="font-serif text-5xl leading-[.78] text-[#f5f0e8] md:text-7xl">Made to<br /><i>move.</i></h2></div></div>
            <div className="relative overflow-hidden md:col-span-5 md:row-span-2"><Image src={editorial[2]} alt="ByChi signature" fill sizes="40vw" className="object-cover" /></div>
            <div className="relative overflow-hidden md:col-span-3"><Image src={editorial[3]} alt="ByChi finish" fill sizes="25vw" className="object-cover" /></div>
          </div>
        </section>

        <section className="relative overflow-hidden bg-black px-5 py-28 md:px-10 md:py-44">
          <div className="absolute inset-0 opacity-45"><Image src={editorial[2]} alt="" fill sizes="100vw" className="object-cover" /></div><div className="absolute inset-0 bg-black/65" />
          <div className="relative z-10 mx-auto max-w-[1760px] text-center"><p className="mb-7 text-[8px] uppercase tracking-[.6em] text-[#c8a45d]">The ByChi House</p><h2 className="font-serif text-[clamp(4rem,10vw,12rem)] leading-[.68] tracking-[-.07em]">Your hair.<br /><i className="text-[#c8a45d]">Your standard.</i></h2><p className="mx-auto mt-10 max-w-xl text-sm leading-7 text-white/60">Choose your piece, speak with a concierge, or step inside the House in Lagos.</p><div className="mx-auto mt-10 flex max-w-xl flex-col gap-3 sm:flex-row"><Link href="/collections" className="flex flex-1 items-center justify-center bg-[#c8a45d] px-6 py-5 text-[9px] font-semibold uppercase tracking-[.32em] text-[#0b0907]">Shop the House</Link><Link href="/book" className="flex flex-1 items-center justify-center border border-white/30 px-6 py-5 text-[9px] uppercase tracking-[.32em] text-white">Book Concierge</Link></div></div>
        </section>
      </main>

      <footer className="border-t border-white/10 bg-[#0b0907] px-5 py-12 text-white/55 md:px-10 md:py-16">
        <div className="mx-auto grid max-w-[1760px] gap-10 md:grid-cols-12"><div className="md:col-span-5"><p className="font-serif text-4xl text-[#f5f0e8]">ByChi Strands</p><p className="mt-5 max-w-sm text-sm leading-7">The House of luxury Vietnamese hair. Lagos · Nigeria.</p></div><div className="grid grid-cols-2 gap-8 text-[8px] uppercase tracking-[.3em] md:col-span-7 md:grid-cols-4"><Link href="/collections">Collections</Link><Link href="/book">Concierge</Link><Link href="/about">The House</Link><Link href="/contact">Contact</Link></div></div>
      </footer>
    </div>
  );
}
