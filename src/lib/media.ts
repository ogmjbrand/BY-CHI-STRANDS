...
export interface Scene {
  ...
  placeholder?: boolean;
  /**
   * True while the file named by `src` is not in the repository yet — the
   * slot is wired, the picture has not been taken. `hasScene()` is false
   * for these, so consumers can render their no-photograph state instead of
   * a broken image.
   */
  pending?: boolean;
}
...
const SCENES = {
   ...
   *
   * Every frame here is a working photograph the house already had, matched
   * to the module it illustrates. None of them is a classroom shot — there
   * is no photography of a masterclass in session yet, so no section claims
   * to show one. Two are marked `placeholder` because they stand in for a
   * frame the campaign shoot should replace, not because the picture is
   * untrue. */
  /** Module 01, Sourcing — the real supplier trip. */
  "academy-sourcing": {
    src: "/services/hair-importation-1.jpeg",
    alt: "Curly raw hair bundles held up for inspection inside a wholesale hair market",
  },
  /** Module 02, Grading — raw bundles examined by hand. */
  "academy-grading": {
    src: "/services/hair-importation-2.jpeg",
    alt: "Raw hair bundles held up to the light at the supplier",
  },
  /** Module 03, Scaling — stock, in the lengths it is bought and sold in. */
  "academy-scaling": {
    src: "/products/boutique-lineup-1.jpg",
    alt: "Four finished curly units on stands, labelled 12, 14, 16 and 18 inches",
  },
  /**
   * Module 04, Marketing — a styled brand portrait, which is what the module
   * is about. Stand-in: the shoot should produce a frame of the work being
   * photographed rather than a finished result of it.
   */
  "academy-marketing": {
   ...
   * for a frame of the house at work.
   */
  "academy-apply": {
  ...
  /* ── Service tiles ──
   *
   * Hair tools is the one service with no photograph of its own. The slot
   * is wired and every consumer reads it through `scene()`, so dropping a
   * file at the path below is the entire job — but the file is not in the
  ...
  "backdrop-collections": {
    src: "/media/posters/imgi_127_0ab18633fccbf196e61d590cb9d1dda2.jpg",
    alt: "",
    placeholder: true,
  },
  "backdrop-standard": {
    src: "/media/posters/imgi_579_725cd77e2084ffd7a3524b831c402af9.jpg",
    alt: "",
    placeholder: true,
  },
  "backdrop-motion": {
  ...
  "backdrop-pieces": {
    src: "/media/posters/imgi_185_da48ecb778a1057cd3f56900226eb6a0.jpg",
    alt: "",
    placeholder: true,
  },
  "backdrop-craft": {
...
/**
 * False while a scene's file is still missing from the repository. Render a
 * typographic fallback rather than an <img> that 404s.
 */
export function hasScene(name: SceneName): boolean {
  return !(SCENES[name] as Scene).pending;
}
